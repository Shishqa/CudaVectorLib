...
int main(int argc, const char *argv[])
{
  ...
  float result = NAN;
  float elapsed = VectorDot(blockSize, a, b, result, ReduceStride);
  printf("s,");
  Report(arrSize, blockSize, elapsed);

  float maxError = fabs(result - X_VAL * Y_VAL * arrSize);
  assert(maxError < 0.001);

  result = NAN;
  elapsed = VectorDot(blockSize, a, b, result, ReducePyramidal);
  printf("r,");
  Report(arrSize, blockSize, elapsed);

  maxError = fabs(result - X_VAL * Y_VAL * arrSize);
  assert(maxError < 0.001);
